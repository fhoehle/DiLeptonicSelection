  ...
  def doDiElectronPath(self,process,pPF,debugIt = False):
    ...
    if debugIt:
      process.myTestAnalyzer = cms.EDAnalyzer("MyCandPrintAnalyzer", src = cms.InputTag(coll), prefix = cms.string("test"+coll),
        quantity = cms.vstring('px', 'py', 'pz', 'energy', 'pt', 'eta', 'phi','gsfTrack.trackerExpectedHitsInner.numberOfLostHits' ,'abs(userFloat("deltaCotTheta"))',' abs(userFloat("deltaDistance"))', 'electronID("simpleEleId90cIso")','(neutralHadronIso + chargedHadronIso + photonIso)/pt','neutralHadronIso ',' chargedHadronIso ',' photonIso','abs(dB)'));
      localPath += process.myTestAnalyzer
